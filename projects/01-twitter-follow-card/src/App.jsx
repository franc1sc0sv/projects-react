...
export const App = ({ name, userName = 'unknow', InitialIsFollowing }) => {
  ...
  const buttonClassName = isFollowing
    ? ' hover:bg-red-700 hover:bg-opacity-10 hover:border-red-700 hover:text-red-700 bg-transparent text-white group'
    : ' bg-white hover:bg-opacity-80 text-[#222]'

  const handleClick = () => setIsFollowing(!isFollowing)

  return (
    <article className='flex gap-2 justify-between text-sm min-w-[300px]'>
