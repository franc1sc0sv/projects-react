...
export function ComboboxPro({ name, control, options }) {
  ...
  const { selectedData, setSelectedData, setQuery, filteredData, query } =
    useCombobox({ options });
      ...
      render={({ field: { onChange } }) => (
        ...
        >
          <div className="relative mt-1">
            <div className="relative flex w-full px-5 overflow-hidden text-left bg-white rounded-lg shadow-md cursor-default h-max focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-teal-300">
              <Combobox.Input
...
function RenderResultsCoins({ coins }) {
  return coins.map((coin, i) => (
    ...
    >
      ...
      <p className="pl-5 font-normal ui-selected:font-medium ui-selected:pl-0">
        {coin.name}
      </p>
    </Combobox.Option>
  ));
